...
class DenseTaskAlignedAssignerXX(nn.Layer):
    ...
    def forward(self,
        ...
        # for i in range(10):
        #     print(anchor_points[i], pred_bboxes[0, i].cpu().numpy(), gt_bboxes[0, i])
        # pred_scores_np = pred_scores.cpu().numpy()
        # print("pred_scores : ", pred_scores_np.shape, pred_scores_np.max(), pred_scores_np.min())

        # compute iou between gt and pred bbox, [B, n, L]
        ious = batch_iou_similarity(gt_bboxes, pred_bboxes)
        
        # ious_np = ious.cpu().numpy()
        # print("ious : ", ious_np.shape, ious_np.max(), ious_np.min())
        ...
        # bbox_cls_scores_np = bbox_cls_scores.cpu().numpy()
        # print("bbox_cls_scores : ", bbox_cls_scores_np.shape, bbox_cls_scores_np.max(), bbox_cls_scores_np.min())
