...
__all__ = ['TaskAlignedDoubleAssigner']


@register
class TaskAlignedDoubleAssigner(nn.Layer):
    """TOOD: Task-aligned One-stage Object Detection
    """

    def __init__(self, strict_topk=5, loose_topk=13, alpha=1.0, beta=6.0, eps=1e-9):
        super(TaskAlignedDoubleAssigner, self).__init__()
    ...
    def forward(self,
        ...
        # for each gt, [B, n, L]
        is_in_strict_topk = gather_topk_anchors(alignment_metrics * is_in_gts, self.strict_topk, topk_mask=pad_gt_mask)
        is_in_loose_topk = gather_topk_anchors(alignment_metrics * is_in_gts, self.loose_topk, topk_mask=pad_gt_mask)

        # select positive sample, [B, n, L]
        mask_positive = is_in_loose_topk * is_in_gts * pad_gt_mask


        mask_positive_strict = is_in_strict_topk * is_in_gts * pad_gt_mask
        ...
        assigned_labels = paddle.where(mask_positive_sum > 0, 
            assigned_labels, paddle.full_like(assigned_labels, bg_index))
    ...
    #     print("Task Aligned Assigner Debug, forward_debug")
    ...
    #         assigned_bboxes = paddle.zeros([batch_size, num_anchors, 4])
    #         assigned_scores = paddle.zeros([batch_size, num_anchors, num_classes])
    #         return assigned_labels, assigned_bboxes, assigned_scores
    ...
    #     gather_topk_input = alignment_metrics * is_in_gts
    ...
    #     # select positive sample, [B, n, L]
    #     mask_positive = is_in_loose_topk * is_in_gts * pad_gt_mask
    #     # strict_mask_positive = is_in_strict_topk * is_in_gts * pad_gt_mask
    ...
    #     mask_positive_sum2 = mask_positive.sum(axis=-2)
    #     print("mask_positive_sum2 : ", mask_positive_sum2.shape)
    #     print("mask_positive_sum2.max() : ", mask_positive_sum2.max().item())
    #     print("mask_positive_sum2.min() : ", mask_positive_sum2.min().item())
